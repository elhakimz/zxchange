...
import type { Timeframe, ChartType } from './types/market.types';
...
function AppContent() {
  ...
  const selectedChartType = useMarketStore((state) => state.selectedChartType);
  ...
  const chartActions = (
    <div className="flex items-center gap-1 bg-bg-void/50 p-0.5 rounded border border-bg-border mr-2 overflow-hidden">
      ...
      <div className="flex items-center gap-0.5 mr-1">
        {chartTypes.map((ct) => (
          <button key={ct.value} onClick={() => setSelectedChartType(ct.value)}
            title={ct.label}
            className={`p-1 rounded transition-colors ${
              selectedChartType === ct.value ? 'bg-brand text-bg-void' : 'text-text-muted hover:text-text-primary'
            }`}>
            <ct.icon size={12} />
          </button>
        ))}
          ...
          >
            {isBarsLoading ? (
              ...
              <div className="flex-1 h-full relative">
                ...
                <MarketChart symbol={selectedSymbol} timeframe={selectedTimeframe} />
              </div>
            )}
          </Panel>
